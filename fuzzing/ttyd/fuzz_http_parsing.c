...
int fuzz_target(const uint8_t *data, size_t size) {
    ...
    if (ret == 0 && pss.buffer != NULL && pss.len > 0) {
        (void)callback_http(&mock_wsi, LWS_CALLBACK_HTTP_WRITEABLE,
                            &pss, NULL, 0);
        /* pss_buffer_free() inside WRITEABLE already freed the buffer —
         * null it so the cleanup below doesn't double-free. */
        pss.buffer = NULL;
    }

    /* Clean up any strdup'd buffer from /token path if WRITEABLE was skipped */
